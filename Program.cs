...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        int dicethrows = int.Parse(Console.ReadLine());

        
        DiceRoller.Roll(dicethrows);
        int[] results = DiceRoller.DiceRolled;

        Console.WriteLine();
        Console.WriteLine("DICE ROLLING SIMULATION RESULTS");
        Console.WriteLine("Each \"*\" represents 1 % of the total number of rolls.");
        Console.WriteLine("Total number of rolls = " + dicethrows);
        Console.WriteLine();

        int counter = 2;
        for (int i=2; i<results.Length; i++) 
        {
            int percentage = (int)((double)results[i] / dicethrows * 100);
            Console.WriteLine($"{counter++}: {new string('*', percentage)}");
        }
